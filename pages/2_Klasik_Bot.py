...
def fetch_crypto_data(exchange_id: str, symbol: str, timeframe: str, limit: int = 1500) -> pd.DataFrame:
    try:
        ...
        # Eğer symbol USDT içermiyorsa veya '/' yoksa formata uygun hale getir
        if "/" not in symbol:
...
def train_and_predict_ai(df: pd.DataFrame, target_candles: int, threshold: float, tp_m: float, sl_m: float):
    ...
    df['oi_change_pct'] = df['oi'].pct_change(periods=5).fillna(0) * 100
    ...
    features_pool = ['rsi', 'macd_hist', 'macd_hist_slope', 'dist_ema50', 'roc', 'adx', 'ema_trend', 
                     'oi_change_pct', 'volume_delta_trend', 'ob_pressure', 'fear_greed', 'dxy']
    ...
    train_df = temp_df.dropna(subset=features_pool).copy()
    train_df = train_df[train_df['is_valid'] == True]
    
    if len(train_df) < 50:
        df['prob_long'], df['prob_short'], df['ai_signal'], df['trade_status'] = 0, 0, 'NONE', 'IDLE'
        df['active_tp'], df['active_sl'] = np.nan, np.nan
        return df, 0.0, features_pool

    selector = SelectKBest(score_func=f_classif, k=min(10, len(features_pool)))
    selector.fit(train_df[features_pool], train_df['target'])
    final_features = [f for f, selected in zip(features_pool, selector.get_support()) if selected]
    
    model = RandomForestClassifier(n_estimators=100, max_depth=6, min_samples_leaf=5, class_weight='balanced', random_state=42)
    model.fit(train_df[final_features], train_df['target'])
    train_acc = accuracy_score(train_df['target'], model.predict(train_df[final_features])) * 100
    
    pred_features = df[final_features].ffill().bfill() 
    probs = model.predict_proba(pred_features)
    classes = list(model.classes_)
    
    df['prob_long'] = pd.Series(probs[:, classes.index(1)] * 100 if 1 in classes else np.zeros(len(probs))).rolling(5).mean().fillna(0).values
    df['prob_short'] = pd.Series(probs[:, classes.index(-1)] * 100 if -1 in classes else np.zeros(len(probs))).rolling(5).mean().fillna(0).values
    
    ai_signals, trade_status = ['NONE'] * len(df), ['IDLE'] * len(df)
    active_tps, active_sls = [np.nan] * len(df), [np.nan] * len(df)
    in_pos, pos_type, tp, sl = False, None, 0.0, 0.0
    
    for i in range(len(df)):
        if in_pos:
            trade_status[i] = f'ACTIVE_{pos_type}'
            active_tps[i], active_sls[i] = tp, sl
...
def main():
    with st.sidebar:
        ...
        auto_ref = st.toggle("🔄 Canlı Akış (5 sn)", value=True)
    ...
    with c5: st.markdown(f'<div class="metric-card"><h4>🧠 ML BAŞARISI</h4><p class="value blue">%{train_acc:.1f}</p></div>', unsafe_allow_html=True)
